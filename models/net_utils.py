...
def separable_conv_bn(x,
    ...
    if depth_activation:
        x = Activation('relu')(x)
    x = Conv2D(n_filters, (1, 1), padding='same', use_bias=False,
               kernel_regularizer=l2(weight_decay), kernel_initializer=kernel_initializer)(x)
    x = BatchNormalization(epsilon=bn_epsilon, momentum=bn_momentum)(x)
    if depth_activation:
        x = Activation('relu')(x)
...
def atrous_spatial_pyramid_pooling(inputs,
    ...
    for i, rate in enumerate(rates):
        atrous_pool_block_i = separable_conv_bn(inputs, 256, rate=rate, depth_activation=True,
...
def load_custom_model(model_path):
    return load_model(model_path, custom_objects={"BilinearUpSampling": BilinearUpSampling})
